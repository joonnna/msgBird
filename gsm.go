...
func (c converter) convert(body string) ([]byte, error) {
	...
	for _, runVal := range body {
		if val, exists := c.table[runVal]; exists {
			...
			return nil, errInvalidGsm
		}
	}
	return ret, nil
}

func generateGsmMap() map[rune]byte {
	ret := make(map[rune]byte)
	for i, v := range gsmTable {
		ret[v] = byte(i)
	}
...
var gsmTable = [0x80]rune{
	...
	/* 0x6F */ 0x006F, /* LATIN SMALL LETTER
	O */
	/* 0x70 */ 0x0070, /* LATIN SMALL LETTER
	P */
	/* 0x71 */ 0x0071, /* LATIN SMALL
	LETTER Q */
	/* 0x72 */ 0x0072, /* LATIN SMALL
	LETTER R */
	/* 0x73 */ 0x0073, /* LATIN
	SMALL LETTER S */
	/* 0x74 */ 0x0074, /* LATIN
